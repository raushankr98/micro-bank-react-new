import React, { FormEvent, useState } from 'react'
import { Link } from 'react-router-dom'
import './Register.css'
import { v4 as uuid } from 'uuid'
import axios from 'axios'

type FormDetails = {
    id: string
    fullName: string,
    dob: string,
    email: string,
    password: string,
    cpassword: string,
    currentServices: Current[],
    availableServices: Current[]
}
type Current = {
    mIcon: string,
    dIcon: string,
    rating: number,
    header: string
}

function Register() {
    const [fullName, setFullName] = useState<string>("")
    const [dob, setDOB] = useState<string>("")
    const [email, setEmail] = useState<string>("")
    const [password, setPassword] = useState<string>("")
    const [cpassword, setCpassword] = useState<string>("")

    const handleSubmit = (e: FormEvent) => {
        e.preventDefault()
        if (checkForm()) {
            const form: FormDetails = {
                id: uuid(),
                fullName: fullName,
                dob: dob,
                email: email,
                password: password,
                cpassword: cpassword,
                currentServices: [{
                    mIcon: "file/bank.svg",
                    dIcon: "file/bank2.png",
                    rating: 0,
                    header: "Current Account"
                },
                {
                    mIcon: "file/payroll.svg",
                    dIcon: "file/payments2.png",
                    rating: 0,
                    header: "Payroll"
                },
                {
                    mIcon: "file/payment.svg",
                    dIcon: "file/payroll2.png",
                    rating: 0,
                    header: "Payments"
                }],
                availableServices: [{
                    mIcon: "file/wallet.svg",
                    dIcon: "file/wallet2.png",
                    rating: 0,
                    header: "Wallet"
                },
                {
                    mIcon: "file/time.svg",
                    dIcon: "file/investment2.png",
                    rating: 0,
                    header: "Investments"
                },
                {
                    mIcon: "file/tax.svg",
                    dIcon: "file/tax2.png",
                    rating: 0,
                    header: "Tax Payment"
                }, {
                    mIcon: "file/money-bag.svg",
                    dIcon: "file/loan2.png",
                    rating: 0,
                    header: "Loan"
                }]
            }
            axios.post('http://localhost:4000/registerAccount', form)
        } else {
            alert("Something went wrong!!")
        }

    }

    const checkForm = (): boolean => {
        if (fullName.trim() !== "" && dob.trim() !== "" && email.trim() !== "" && password.trim() !== "" && cpassword.trim() !== "") {
            if (password === cpassword) {
                return true
            }
        }
        return false
    }

    return (
        <div className="main_container">
            <div className="container_img_div">
            </div>

            {/*  register form   */}
            <div className="form_div">
                <div style={{ paddingTop: "20px" }} className="inner_form_div">
                    <h3 id="login_name">Create an Account</h3>
                    <form onSubmit={handleSubmit} id="formData">
                        <label className="label" >Full Name</label><br />
                        <input type="text" id="fullName" onChange={(e) => (setFullName(e.target.value))} required /><br /><br />

                        <label className="label" >Date of Incorporation</label><br />
                        <input type="date" id="openDate" onChange={(e) => (setDOB(e.target.value))} required /><br /><br />

                        <label className="label" >Email</label><br />
                        <input type="email" id="email" onChange={(e) => (setEmail(e.target.value))} required /><br /><br />

                        <label className="label" >Password</label><br />
                        <input type="password" id="password" minLength={8} onChange={(e) => (setPassword(e.target.value))} required />
                        <span className="hide" id="hide1" style={{ margin: "-40px" }}><svg width="22" height="19"
                            viewBox="0 0 22 19" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <path
                                d="M11 4C13.76 4 16 6.24 16 9C16 9.65 15.87 10.26 15.64 10.83L18.56 13.75C20.07 12.49 21.26 10.86 21.99 9C20.26 4.61 15.99 1.5 10.99 1.5C9.59 1.5 8.25 1.75 7.01 2.2L9.17 4.36C9.74 4.13 10.35 4 11 4ZM1 1.27L3.28 3.55L3.74 4.01C2.08 5.3 0.78 7.02 0 9C1.73 13.39 6 16.5 11 16.5C12.55 16.5 14.03 16.2 15.38 15.66L15.8 16.08L18.73 19L20 17.73L2.27 0L1 1.27ZM6.53 6.8L8.08 8.35C8.03 8.56 8 8.78 8 9C8 10.66 9.34 12 11 12C11.22 12 11.44 11.97 11.65 11.92L13.2 13.47C12.53 13.8 11.79 14 11 14C8.24 14 6 11.76 6 9C6 8.21 6.2 7.47 6.53 6.8ZM10.84 6.02L13.99 9.17L14.01 9.01C14.01 7.35 12.67 6.01 11.01 6.01L10.84 6.02Z"
                                fill="black" />
                        </svg>
                        </span>
                        <br /><br />

                        <label className="label" >Confirm Password</label><br />
                        <input type="password" id="confirmPassword" minLength={8} onChange={(e) => (setCpassword(e.target.value))} required />
                        <span className="hide" id="hide2" style={{ margin: "-40px" }}>
                            <svg width="22" height="19" viewBox="0 0 22 19"
                                fill="none" xmlns="http://www.w3.org/2000/svg">
                                <path
                                    d="M11 4C13.76 4 16 6.24 16 9C16 9.65 15.87 10.26 15.64 10.83L18.56 13.75C20.07 12.49 21.26 10.86 21.99 9C20.26 4.61 15.99 1.5 10.99 1.5C9.59 1.5 8.25 1.75 7.01 2.2L9.17 4.36C9.74 4.13 10.35 4 11 4ZM1 1.27L3.28 3.55L3.74 4.01C2.08 5.3 0.78 7.02 0 9C1.73 13.39 6 16.5 11 16.5C12.55 16.5 14.03 16.2 15.38 15.66L15.8 16.08L18.73 19L20 17.73L2.27 0L1 1.27ZM6.53 6.8L8.08 8.35C8.03 8.56 8 8.78 8 9C8 10.66 9.34 12 11 12C11.22 12 11.44 11.97 11.65 11.92L13.2 13.47C12.53 13.8 11.79 14 11 14C8.24 14 6 11.76 6 9C6 8.21 6.2 7.47 6.53 6.8ZM10.84 6.02L13.99 9.17L14.01 9.01C14.01 7.35 12.67 6.01 11.01 6.01L10.84 6.02Z"
                                    fill="black" />
                            </svg>
                        </span>
                        <br />

                        <input className="submit_btn" type="submit" value="Create an Account" />
                    </form>
                    <p style={{ marginLeft: "45px", marginTop: "10px" }}>Already have an account? <Link id="a_create_account"
                        to="/" >Login</Link></p>
                </div>
            </div>
        </div>
    )
}

export default Register
